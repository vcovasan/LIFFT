...
def getBackground(img):
	...
	for c in cnts:
		...
		for j in cyList:
			if abs(yy-j)<5:
				yFlag = True
		if xFlag==False and yFlag==False and vertices<8 and area>5000 and area<9000 and aspect>0.3 and aspect<0.8 and imgmean>8 and imgmean<30:
			maskFlag = True
			cxList.append(xx)
			cyList.append(yy)
...
def getScrews(img):
	...
	for c in cnts:
		...
		for j in cyList:
			if abs(yy-j)<5:
				yFlag = True
		if xFlag==False and yFlag==False and vertices<8 and area>5 and area<150 and aspect>0.4 and aspect<1.5 and imgmean>6 and imgmean<40 and cntmean<140:
			cxList.append(xx)
			cyList.append(yy)
...
def getDrill(img):
	...
	ret1,img1_th = cv2.threshold(img1_gray,55,255,cv2.THRESH_BINARY)
	cv2.imshow('img1_thresh',img1_th)
	imr1 = np.array(img1_th[178,:])
	row_mean = imr1.mean()
	...
	if row_mean<255:
		...
		if w1<40 and imr1_min>230 and imr1_max<330:
			cv2.line(img,(imr1_min,178),(imr1_max,178),(0,255,0),3)
			cv2.putText(img,str(w1),(imr1_max+int(w1),178),cv2.FONT_HERSHEY_SIMPLEX,0.4,(0,255,0),1)		
			# cv2.imshow('img1_th',img1_th)
	return img



cap = cv2.VideoCapture('H33.mp4')
i = 0
while(True):    
